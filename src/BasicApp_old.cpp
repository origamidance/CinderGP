...
void BasicApp::keyDown(MouseEvent event) {}
...
void BasicApp::update() {
  ...
  static const Object* selection = &mObjects[1];
  {
    ...
    if (ui::Button("Add")) {
      ...
      mObjects.push_back(
          {"Object" + to_string(objCount++), Color::white(), vec2(0.0f)});
    }
    if (selection) {
      ui::SameLine();
      if (ui::Button("Remove")) {
        auto it =
            std::find_if(mObjects.begin(), mObjects.end(),
                         [](const Object& obj) { return &obj == selection; });
        if (it != mObjects.end()) {
          mObjects.erase(it);
          selection = nullptr;
        }
  ...
  if (selection) {
    ...
    if (ui::DragFloat2("Position", &pos[0]))
...
void BasicApp::draw() {
  ...
  for (auto object : mObjects) {
    ...
    gl::drawStringCentered(object.mName,
                           vec2(ivec2(object.getPosition())) - vec2(0, 5),
                           ColorA::black());
  }
}
