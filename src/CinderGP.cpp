...
GeometryApp::GeometryApp()
    : mCapsuleRadius(0.5f), mCapsuleLength(1), mConeRatio(0.5f),
      mHelixRatio(0.25f), mHelixTwist(0), mHelixOffset(0), mHelixCoils(3),
      mRingWidth(0.25f), mRoundedRectRadius(0.2f), mTorusTwist(0),
      mTorusOffset(0), mTorusRatio(0.25), mTorusKnotP(2), mTorusKnotQ(5),
      mTorusKnotRadius(0.15f), mTorusKnotScale(1, 0.2, 1) {}
...
void GeometryApp::mouseDrag(MouseEvent event) { mCamUi.mouseDrag(event); }
...
void GeometryApp::createGeometry() {
  ...
  case CAPSULE:
    ...
    case DEFAULT:
      loadGeomSource(
          ...
          geom::WireCapsule().radius(mCapsuleRadius).length(mCapsuleLength));
      break;
    case LOW:
      loadGeomSource(
          geom::Capsule()
  ...
  case CYLINDER:
    ...
    case DEFAULT:
      ...
      break;
    case LOW:
      loadGeomSource(geom::Cylinder().subdivisionsAxis(6).subdivisionsCap(1),
                     geom::WireCylinder());
      break;
    case HIGH:
  ...
  case RING:
    ...
    case DEFAULT:
      loadGeomSource(
          geom::Ring().width(mRingWidth).subdivisions(24),
          geom::WireCircle().subdivisions(24).radius(1 + 0.5f * mRingWidth));
      break;
    case LOW:
      loadGeomSource(
    ...
    case HIGH:
      loadGeomSource(
          geom::Ring().width(mRingWidth).subdivisions(120),
          geom::WireCircle().subdivisions(120).radius(1 + 0.5f * mRingWidth));
      break;
    }
    break;
  case VBOMESH:
...
CINDER_APP(GeometryApp, RendererGl(RendererGl::Options().msaa(16)),
           prepareSettings)
/**
 * @name initUI - Initializes the Imgui
 * @return void
 */
void GeometryApp::initUI() { ui::initialize(); }
/**
